...
/* ################################################################# */
{
	...
	double du, dv, time, TOTAL_TIME = 10.0;
...
/***** Purpose: To initilise all parameters. ******/
	...
	for (I = 0; I <= NPI + 1; I++) {
		...
		for (J = 0; J <= NPJ + 1; J++) {
			j = J;
			u      [i][J] = U_IN; //*1.5*(1.-sqr(2.*(y[J]-YMAX/2.)/YMAX));     /* Velocity in x-direction */ //1.5... uitzetten als je rechte instroom wil hebben
...
/***** Purpose: Specify boundary conditions for a calculation ******/
	...
	for (J = 0; J <= NPJ + 1; J++) {
		...
		u[1][J] = U_IN; /* inlet rechte instroom */
//		u[1][J] = U_IN*1.5*(1.-sqr(2.*(y[J]-YMAX/2.)/YMAX)); /* inlet parabolische instroom */
	} /* for J */

/* T gradient set tot zero at upper and lower outlet */
	for (I = 0; I <= NPI + 1; I++) {
		/* Temperature at the walls in Kelvin */
...
/*
			...
			if (i > b_ship + (countmax+1) && i <= b_ship + (countmax+2) + l_ship  && J >= 0.5*NPJ - count2 && J <= 0.5*NPJ + count2 )
				SP[i][J] = -LARGE; 
...
/*
			...
			*/
			...
			if ( (i > b_ship + count1 && i <= b_ship + (count1) && J == 0.5*NPJ + (count2+1)) || 
...
/***** Purpose: To calculate the coefficients for the v equation. ******/
	...
	for (I = Istart; I <= Iend; I++) {
		...
		for (J = Jstart; J <= Jend; J++) {
			...
			if( (I > b_ship + count1 && I <= b_ship + (count1) && (J == 0.5*NPJ - (count2+1) || J == 0.5*NPJ + (count2+1))) || 
...
/***** Purpose: Calculate uplus, yplus, vplus, xplus and tw  ******/
	...
	viscosity();
	//over J laten lopen voor de turbulente grenslaag aan te zetten en y[J] - y_v[j]
	for (I = 0; I <= NPI; I++){
	    i=I;
